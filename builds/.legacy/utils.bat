...
:CheckBuildVersion
    set "_target_dir=%~1"
    for %%I in ("%_target_dir%") do set "_target_dir=%%~fI"
    set "_target_ver=%~2"
    set "_ver_file=%_target_dir%\.ver"
    set "_build_needed=1"

    if not exist "%_ver_file%" exit /b 0

    set /p _current_ver=<"%_ver_file%"
    if "%_current_ver%"=="%_target_ver%" (
        call :LogInfo "Build version %_target_ver% matches. Skipping build."
        set "_build_needed=0"
    )
    exit /b 0
...
:WriteBuildVersion
    set "_target_dir=%~1"
    set "_target_ver=%~2"
    if not exist "%_target_dir%" md "%_target_dir%" >nul 2>&1
    (echo %_target_ver%)> "%_target_dir%\.ver"
    exit /b 0
